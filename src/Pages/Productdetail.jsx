...
function Productdetail() {
 ...
 return (
  <Box className='w-full place-items-center px-2  min-h-screen mt-[120px]'>
   <Box className='w-full border-2 p-3 text-center justify-center md:justify-start md:text-first flex flex-col bg-black  md:flex-row gap-4 border-green-500 h-[570px] lg:h-[380px] lg:w-[60%]'>
      ...
      <Box className='space-y-2 text-green-500 font-bold place-items-center text-center md:text-first md:place-items-start w-full  md:w-[50%]'>
        ...
        <Button onClick={()=>dispatch(decreaseproduct(productcontain?.price))} className="border-2 rounded-l-full border-green-400 w-14 h-7  mr-2">-</Button>{productcontain?.quantity}
        <Button onClick={()=>dispatch(increaseproduct(productcontain?.price))}className="border-2 rounded-r-full border-green-400 w-14 h-7 ml-2">+</Button>
